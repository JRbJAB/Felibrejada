...
var FBR_CARTE_MENTALE_DYNAMIC_VERSION = 'v1.2.0-dynamic-fused-20260711';
...
/**
 * Carte mentale dynamique — fonctions publiques UI.
 * À appeler depuis le menu Sheet ou sidebar, pas depuis l’éditeur pour les fonctions UI.
 */

function FELIBREE_openCarteMentaleComV1Fullscreen() {
  try {
    return FBR_CARTE_MENTALE_UI_showFullscreen_();
  } catch (err) {
    var message = String(err && err.message ? err.message : err);
    if (message.indexOf('Cannot call SpreadsheetApp.getUi() from this context') !== -1) {
      Logger.log('UI indisponible depuis l’éditeur Apps Script. Ouvrir le Google Sheet puis utiliser le menu carte mentale dynamique.');
      return {
        ok: false,
...
function FELIBREE_openCarteMentaleComFullscreen() {
  // Alias stable pour le menu existant : la carte dynamique v1 devient la carte principale.
  return FELIBREE_openCarteMentaleComV1Fullscreen();
}

function FELIBREE_openCarteMentaleComDynamicFullscreen() {
  return FELIBREE_openCarteMentaleComV1Fullscreen();
}

function FBR_CARTE_MENTALE_UI_showFullscreen_() {
  var state = FBR_CARTE_MENTALE_DATA_buildComState_('VIEW');
  var html = FBR_CARTE_MENTALE_RENDER_evaluateTemplate_(state);
  SpreadsheetApp.getUi().showModelessDialog(html, '🧠 Carte mentale COM dynamique — Félibrée 2027');
  return {
    ok: true,
    version: FBR_CARTE_MENTALE_DYNAMIC_VERSION,
...
function FELIBREE_carteMentaleComV1Diagnostic() {
  var cfg = FBR_CARTE_MENTALE_CFG_get_();
  var result = {
    ok: true,
    version: FBR_CARTE_MENTALE_DYNAMIC_VERSION,
  ...
  try {
    ...
    result.ok = false;
    result.templateError = 'Fichier HTML Apps Script introuvable : ' + cfg.TEMPLATE_HTML;
  }
...
function FELIBREE_refreshCarteMentaleComV1DryRun() {
...
function FELIBREE_refreshCarteMentaleComV1Apply() {
  var cfg = FBR_CARTE_MENTALE_CFG_get_();
  var allow = PropertiesService.getScriptProperties().getProperty(cfg.PROP_ALLOW_WRITE) === 'TRUE';
  if (!allow) {
    return {
      ok: false,
      dryRun: false,
      ...
      message: 'Définir ' + cfg.PROP_ALLOW_WRITE + '=TRUE pour exporter la carte mentale dynamique.'
    };
  }

  var state = FBR_CARTE_MENTALE_DATA_buildComState_('APPLY');
  var html = FBR_CARTE_MENTALE_RENDER_buildStandaloneHtml_(state);
  var fileName = 'carte_mentale_COM_2027_dynamic_' + Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyyMMdd_HHmmss') + '.html';

  var folderId = PropertiesService.getScriptProperties().getProperty(cfg.PROP_EXPORT_FOLDER_ID);
  var file;
  if (folderId) {
    file = DriveApp.getFolderById(folderId).createFile(fileName, html, MimeType.HTML);
  } else {
    file = DriveApp.createFile(fileName, html, MimeType.HTML);
  }

  FBR_CARTE_MENTALE_REGISTRY_log_('CARTE_MENTALE_COM_EXPORT_APPLY', 'OK', file.getUrl(), state);
  FBR_CARTE_MENTALE_REGISTRY_updateMapRegistry_(file.getUrl(), state);
